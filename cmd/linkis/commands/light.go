...
var LightCmd = &cobra.Command{
	Use:   "light [chainID]",
	Short: "Run a light client proxy server, verifying Linkis rpc",
	Long: `Run a light client proxy server, verifying Linkis rpc.

All calls that can be tracked back to a block header by a proof
will be verified before passing them back to the caller. Other than
that, it will present the same interface as a full Linkis node.
...
func init() {
	...
	LightCmd.Flags().StringVarP(&primaryAddr, "primary", "p", "",
		"connect to a Linkis node at this address")
	LightCmd.Flags().StringVarP(&witnessAddrsJoined, "witnesses", "w", "",
		"Linkis nodes to cross-check the primary node, comma-separated")
	...
	if primaryAddr == "" { // check to see if we can start from an existing state
		...
		if primaryAddr == "" {
			return errors.New("no primary address was provided nor found. Please provide a primary (using -p)." +
				" Run the command: linkis light --help for more information")
		}
	} else {
