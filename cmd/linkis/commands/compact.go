...
var CompactGoLevelDBCmd = &cobra.Command{
	...
	Short: "force compacts the Linkis storage engine (only GoLevelDB supported)",
	Long: `
...
func compactGoLevelDBs(rootDir string, logger log.Logger) {
	...
	for _, dbName := range dbNames {
		...
		go func() {
			...
			if err != nil {
				logger.Error("failed to initialize Linkis db", "path", dbPath, "err", err)
				return
			}
			defer store.Close()
			...
			if err != nil {
				logger.Error("failed to compact Linkis db", "path", dbPath, "err", err)
			}
		}()
	}
	wg.Wait()
