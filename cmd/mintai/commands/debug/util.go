...
// dumpStatus gets node status state dump from the Linkis RPC and writes it
...
// dumpNetInfo gets network information state dump from the Linkis RPC and
...
// dumpConsensusState gets consensus state dump from the Linkis RPC and
// writes it to file. It returns an error upon failure.
...
// copyWAL copies the Linkis node's WAL file. It returns an error if the
...
// copyConfig copies the Linkis node's config file. It returns an error if
