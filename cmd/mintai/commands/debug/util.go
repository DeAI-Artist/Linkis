...
// dumpNetInfo gets network information state dump from the MintAI RPC and
...
// dumpConsensusState gets consensus state dump from the MintAI RPC and
// writes it to file. It returns an error upon failure.
...
// copyWAL copies the MintAI node's WAL file. It returns an error if the
...
// copyConfig copies the MintAI node's config file. It returns an error if
