...
contract RewardsDistribution is Owned, IRewardsDistribution {
    ...
    function setMAIProxy(address _MAIProxy) external onlyOwner {
        MAIProxy = _MAIProxy;
    }
    ...
    function distributeRewards(uint amount) external returns (bool) {
        ...
        for (uint i = 0; i < distributions.length; i++) {
            if (distributions[i].destination != address(0) && distributions[i].amount != 0) {
                ...
                // Transfer the MAI
                IERC20(MAIProxy).transfer(distributions[i].destination, distributions[i].amount);

            }
        }

        // After all ditributions have been sent, send the remainder to the RewardsEscrow contract
        IERC20(MAIProxy).transfer(rewardEscrow, remainder);
